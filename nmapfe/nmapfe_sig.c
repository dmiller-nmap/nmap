...
{
	char ch[50];
	char *filename, *text_contents, *string, *new;
	FILE *file;
	GdkFont *fixed, *bold;
  	GdkColormap *cmap;
	...
	if((file = fopen(filename, "r"))){
		...
		if(view_type == 1){
		new = strsep(&string, " ");
		gtk_text_insert(GTK_TEXT(MW->output), fixed, NULL, NULL, new, -1); 
