...
public:
  bool timedout; /* Have we run out of time for the scan? */
...
GroupScanStats::GroupScanStats(UltraScanInfo *UltraSI) {
  ...
  USI = UltraSI;
  init_ultra_timing_vals(&timing, TIMING_GROUP, USI->numIncompleteHosts(), &(USI->perf));
  timedout = false;
...
HostScanStats::HostScanStats(Target *t, UltraScanInfo *UltraSI) { 
  ...
  numprobes_sent = 0;
  numpings_sent = 0;
  init_ultra_timing_vals(&timing, TIMING_HOST, 1, &(USI->perf));
  bench_tryno = 0;
...
HostScanStats::~HostScanStats() {
  ...
  for(probeI = probe_bench.begin(); probeI != probe_bench.end(); 
      probeI = next) {
    next = probeI;
    next++;
    destroyOutstandingProbe(probeI);
...
bool HostScanStats::sendOK(struct timeval *when) {
  ...
  for(probeI = probes_outstanding.begin(); probeI != probes_outstanding.end();
    ...
    if (!(*probeI)->timedout) {
      ...
      if (firstgood || 
	  TIMEVAL_SUBTRACT(probe_to, USI->now) < TIMEVAL_SUBTRACT(earliest_to, USI->now)) {
...
static void init_perf_values(struct ultra_scan_performance_vars *perf) {
  ...
  perf->low_cwnd = 1;
  perf->host_initial_cwnd = 10;
  perf->group_initial_cwnd = 10;
  perf->max_cwnd = 300;
  perf->quick_incr = 1;
  perf->cc_incr = 1;
  ...
  perf->group_drop_ccthresh_divisor = 2.0;
  perf->host_drop_ccthresh_divisor = 2.0;
  perf->tryno_cap = 12;
}
...
void UltraScanInfo::Init(vector<Target *> &Targets, struct scan_lists *pts, stype scantp) {
  ...
  gstats->timeout = Targets.back()->host_timeout;
  pd = NULL;
...
bool UltraScanInfo::sendOK(struct timeval *when) {
  ...
  for(host = incompleteHosts.begin(); host != incompleteHosts.end(); host++) {
    if ((*host)->sendOK(&tmptv)) {
      lowhtime = tmptv;
      hgood = true;
      break;
    }
    if (hostno == 0 || TIMEVAL_SUBTRACT(lowhtime, tmptv) > 0)
      lowhtime = tmptv;
    hostno++;
  }
...
int determineScanGroupSize(int hosts_scanned_so_far, 
  ...
  if (o.UDPScan())
    ...
    groupsize = MAX(100 / ports->tcp_count, 20);
...
void ultrascan_adjust_times(UltraScanInfo *USI, HostScanStats *hss, 
  ...
  if (probe->tryno > 0 || !rcvdtime) {
    ...
    if (USI->gstats->timing.cwnd <= USI->gstats->timing.ccthresh) {
      /* In quick start mode */
      USI->gstats->timing.cwnd += USI->perf.quick_incr;
    } else {
      /* Congestion control mode */
      USI->gstats->timing.cwnd += USI->perf.cc_incr / USI->gstats->timing.cwnd;
    }
  }
  ...
  if (oldbad > 10 && (oldbad / ((double) oldbad + oldgood) > 0.15)) {
...
/* Tries to get one *good* (finishes a probe) pcap response by the
   (absolute) tiem given in stime.  Even if stime is now, try an
...
static void begin_sniffer(UltraScanInfo *USI, vector<Target *> &Targets) {
  ...
  set_pcap_filter(Targets[0], USI->pd, flt_icmptcp, pcap_filter);
...
void ultra_scan(vector<Target *> &Targets, struct scan_lists *ports, 
  ...
  time_t starttime;
  if (Targets.size() == 0 || Targets.back()->timedout) {
    return;
  }
  ...
  while(USI->numIncompleteHosts() != 0 && !USI->gstats->timedout) {
    doAnyPings(USI);
  ...
  if (o.verbose && !USI->gstats->timedout) {
    long secs =  USI->now.tv_sec - USI->SPM->begin.tv_sec;
    log_write(LOG_STDOUT, "The %s took %ld %s to scan %d total ports.\n", scantype2str(scantype), secs, (secs == 1)? "second" : "seconds", USI->gstats->numports * Targets.size());
  }
  delete USI;
}
...
static void posportupdate(Target *target, struct portinfo *current, 
  ...
  if (current->state != PORT_OPEN && current->state != PORT_CLOSED &&
      current->state != PORT_FILTERED && current->state != PORT_TESTING) {
...
|| (o.af() == AF_INET6 && htons(s_in6->sin6_port) == current->portno)
	...
	case EACCES:
	  /* Apparently this can be caused by dest unreachable admin
	     prohibited messages sent back, at least from IPv6
	     hosts */
	  posportupdate(target, current, NULL, trynum, scan, ss, CONNECT_SCAN, PORT_FILTERED, pil, csi);
       break;
...
void pos_scan(Target *target, u16 *portarray, int numports, stype scantype) {
  ...
  if (target->ports.state_counts_tcp[PORT_OPEN] + target->ports.state_counts_tcp[PORT_CLOSED] + target->ports.state_counts_tcp[PORT_FILTERED] == numports && scantype == SYN_SCAN) {
    if (o.debugging)
  ...
  do {
	...
	for( current = pil.firewalled; current ; 
	     current = (current->next > -1)? &scan[current->next] : NULL) {
	  target->ports.addPort(current->portno, IPPROTO_TCP, NULL, PORT_FILTERED);
	}
	...
	for(portno = 0; portno < 65536; portno++)
	  {
	    ...
	    if (current_port_tmp) {
	      ...
	      current_port_tmp->state = PORT_FILTERED;
	      target->ports.state_counts[PORT_OPEN]--;
	      target->ports.state_counts[PORT_FILTERED]++;
	      target->ports.state_counts_udp[PORT_OPEN]--;
	      target->ports.state_counts_udp[PORT_FILTERED]++;
	    }
	  }
      }
    } else { 
	...
	for(portno = 0; portno < 65536; portno++)
	  {
	    ...
	    if (current_port_tmp) {
	      ...
	      target->ports.state_counts_tcp[PORT_FILTERED]++;
	    }
	  }
      }
    }
